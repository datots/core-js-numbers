...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    return b === 0
      ? 'Infinite roots (identity equation)'
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const magniture1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magniture2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  const angle = Math.acos(dotProduct / (magniture1 * magniture2));
